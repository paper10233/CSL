...
def main(opt):
    ...
    for epoch in range(start_epoch + 1, opt.num_epochs + 1):
        ...
        log_dict_train, _, scale = trainer.train(epoch, train_loader,scale)
        ...
        if epoch % 1 == 0:
            ...
            trainer_val = Trainer(opt, model, optimizer)
            trainer_val.set_device(opt.gpus, opt.chunk_sizes, opt.device)
            print("------------------------------evaluate:-------------------------------------")
            log_dict_val, _,scale = trainer_val.val(epoch, val_loader,scale)
            logger.write('epoch: {} |'.format(epoch))
